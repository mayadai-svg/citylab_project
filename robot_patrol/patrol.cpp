...
// 4- Implement patrol algorithm
    // 4a- Move robot forward until an obstacle is detected in front of the robot closer than 35cm
    // 4b- When the robot detects an obstacle in front, apply the following logic in order to avoid it:
        // You need to get the rays corresponding to those 180º.
        // Identify the ray with the greatest distance (which is not an inf)
        // Get its corresponding angle with respect to the robot's front X axis
...
// 5- Move robot to safest area
    // To move the robot toward the safe direction, you need to publish the appropriate values to the robot’s velocity topic.
    // 5a- Inside the patrol.cpp, create a publisher to the /cmd_vel topic.
    // 5b- Create a control callback loop that runs at 10 Hz.
    // 5c- At every iteration of the control loop, publish the proper velocity command to the topic, based on the values detected by the laser.
